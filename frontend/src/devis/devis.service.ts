...
export class DevisService {
  ...
  async remove(id: number, companyId: number) {
    ...
    const [factureCount, bonCommandeCount, commandeFournisseurCount] =
      await Promise.all([
        this.prisma.facture.count({ where: { devisId: id } }),
        this.prisma.bonCommande.count({ where: { devisId: id } }),
        this.prisma.commandeFournisseur.count({ where: { devisId: id } }),
      ]);

    if (factureCount > 0) {
      throw new ConflictException(
        'Impossible de supprimer ce devis : une ou plusieurs factures y sont rattachees',
      );
    }
    if (bonCommandeCount > 0) {
      throw new ConflictException(
        'Impossible de supprimer ce devis : un bon de commande y est rattache',
      );
    }
    if (commandeFournisseurCount > 0) {
      throw new ConflictException(
        'Impossible de supprimer ce devis : une ou plusieurs commandes fournisseur y sont rattachees',
      );
    }
