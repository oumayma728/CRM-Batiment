  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Lignes du devis</label>
        <button
          type="button"
          onClick={add}
          className="flex items-center gap-1 rounded-lg bg-primary-50 dark:bg-primary-900/30 px-2.5 py-1 text-xs font-medium text-primary-700 dark:text-primary-400 transition hover:bg-primary-100 dark:hover:bg-primary-900/50"
        >
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        {lignes.map((ligne, index) => (
          <div key={ligne.id} className={cn(
            'grid grid-cols-12 gap-2 p-3 items-start',
            index > 0 && 'border-t border-slate-100 dark:border-slate-700',
            'bg-white dark:bg-slate-800',
          )}>
            {/* Désignation */}
            <div className="col-span-5">
              {index === 0 && <p className="mb-1 text-[10px] font-semibold uppercase text-slate-400 dark:text-slate-500">Désignation</p>}
              <input
                type="text"
                required
                placeholder="Description du produit/service"
                value={ligne.designation}
                onChange={e => update(ligne.id, 'designation', e.target.value)}
                className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 focus:outline-none"
              />
            </div>
            {/* Quantité */}
            <div className="col-span-2">
              {index === 0 && <p className="mb-1 text-[10px] font-semibold uppercase text-slate-400 dark:text-slate-500">Qté</p>}
              <input
                type="number"
                min="0"
                step="0.01"
                ...
                onChange={e => update(ligne.id, 'quantite', e.target.value)}
                className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 focus:outline-none"
              />
            </div>
            {/* Prix unitaire */}
            <div className="col-span-3">
              {index === 0 && <p className="mb-1 text-[10px] font-semibold uppercase text-slate-400 dark:text-slate-500">Prix HT</p>}
              <input
                type="number"
                min="0"
                step="0.01"
                ...
                onChange={e => update(ligne.id, 'prixUnitaire', e.target.value)}
                className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 focus:outline-none"
              />
            </div>
            {/* Unité + delete */}
            <div className="col-span-2 flex items-end gap-1">
              <div className="flex-1">
                {index === 0 && <p className="mb-1 text-[10px] font-semibold uppercase text-slate-400 dark:text-slate-500">Unité</p>}
                <input
                  type="text"
                  placeholder="u"
                  value={ligne.unite}
                  onChange={e => update(ligne.id, 'unite', e.target.value)}
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-2 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 focus:outline-none"
                />
              </div>
              <button
                type="button"
                ...
                className="mb-0 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-slate-400 transition hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:text-rose-500 disabled:opacity-30"
              >
                <X size={14} />
              </button>
            </div>
      ...
      <div className="flex justify-end pr-1">
        <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
          Total HT estimé : <span className="text-primary-700 dark:text-primary-400">{formatCurrency(total)}</span>
        </span>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 transition hover:bg-slate-100 dark:hover:bg-slate-700"
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 dark:bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 dark:hover:bg-primary-700 shadow-sm"
          >
            <Plus size={16} />
            Nouveau devis
          </button>
